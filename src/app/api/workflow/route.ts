...
async function saveRecord(sb:Supabase, record:WorkflowRecord, before:WorkflowRecord) {
  ...
  if(newComments.length) { const q=await sb.from('workflow_comments').insert(newComments.map(x=>({id:x.id,task_id:record.taskId,author_name:x.authorName,author_role:x.authorRole,body:x.body,visibility:x.visibility,timestamp:x.timestamp||null,created_at:x.createdAt}))); if(q.error) throw q.error; }
